...
%is reported 0.74 for our model, and the previous state-of-the-art is 0.72. For MDB-Drums dataset, The F-measure score is reported 0.71 for ours and 0.67 for the previous one. Based on our experimental results, the proposed dataset is able to support the training of the proposed model and lead to promising performances.
...
%(a dataset with synchronized audio and symbolic annotation with 12h23m) and compared with the state-of-art beat tracking Python libraries (madmom, librosa, and pretty\_midi). Results show that the proposed model outperforms madmom (syn), librosa (syn), pretty\_midi by 4.5\%, 13.4\%, and 35.9\%, respectively. As for downbeat tracking, BLSTM-Attn outperforms madmom (syn) and pretty\_midi by 14.7\% and 26.5\%. Such a comparison of performance indicates the technical challenges in symbolic music beat tracking and the importance of data-driven approaches to symbolic music beat tracking. 
