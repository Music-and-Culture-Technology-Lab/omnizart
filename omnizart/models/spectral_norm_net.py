...
def transpose_residual_block(x, channels, to_down=True, use_bias=True, spectral_norm=True, scope='transblock'):
    with tf.name_scope(scope):
        init_channel = shape_list(x)[-1]
        with tf.name_scope('res1'):
            out = tf.keras.layers.ELU()(x)
            out = conv_sa(out, channels, kernel=(3, 3), strides=(1, 1), pad=1, pad_type='reflect', spectral_norm=spectral_norm, scope=f"{scope}_res1")

        with tf.name_scope('res2'):
            out = tf.keras.layers.ELU()(out)
            out = conv_sa(out, channels, kernel=(3, 3), strides=(1, 1), pad=1, pad_type='reflect', spectral_norm=spectral_norm, scope=f"{scope}_res2")
            if to_down :
                out = down_sample(out)

        if to_down or init_channel!=channels :
            with tf.name_scope('shortcut'):
                x = conv_sa(x, channels, kernel=(1, 1), strides=(1, 1), spectral_norm=spectral_norm, scope=f"{scope}_shortcut")
                if to_down :
                    x = down_sample(x)
...
def drum_model(out_classes, mini_beat_per_seg, layer_num=3):
    with tf.name_scope('transcription_model'):
        ...
        mix_1 = dense_1 + dense_2*0.25

        dense_3 = tf.keras.layers.Dense(2**10, activation='elu', name='mdl_nn_mlp_o3')(mix_1)
        mix_2 = dense_3 + mix_1*0.25

        dense_4 = tf.keras.layers.Dense(2**10, activation='elu', name='mdl_nn_mlp_of2')(mix_2)
        dense_5 = tf.keras.layers.Dense(out_classes*mini_beat_per_seg, activation='tanh', name='mdl_nn_mlp_of3')(dense_4)
