...
def conv_block(input_tensor, channel, kernel_size, strides=(2, 2), dilation_rate=1, dropout_rate=0.4):
    ...
    input_tensor = Conv2D(channel, kernel_size, strides=strides, dilation_rate=dilation_rate, padding="same")(
        input_tensor
    )

    input_tensor = BatchNormalization()(Activation("relu")(input_tensor))
    input_tensor = Dropout(dropout_rate)(input_tensor)
    input_tensor = Conv2D(channel, kernel_size, strides=(1, 1), dilation_rate=dilation_rate, padding="same")(
        input_tensor
    )
...
def multi_head_attention(x, name=None, **kwargs):
    """Wrapper of multi-head attention."""
    return Lambda(lambda a: multihead_attention(a, **kwargs), name=name)(x)


def semantic_segmentation(
    feature_num=128, timesteps=256, multi_grid_layer_n=1, multi_grid_n=3, ch_num=1, prog=False, out_class=2
):
...
def semantic_segmentation_attn(
    feature_num=128, timesteps=256, multi_grid_layer_n=1, multi_grid_n=3, ch_num=1, prog=False, out_class=2
):
    ...
    feature = en_l4
    f_attn = multi_head_attention(feature, name="attn_1", out_channel=64, query_shape=(100, 32))
    f_attn = BatchNormalization()(Activation("relu")(f_attn))
    f_attn = multi_head_attention(f_attn, name="attn-2", d_model=64, out_channel=128, query_shape=(64, 16))
    f_attn = BatchNormalization()(Activation("relu")(f_attn))
